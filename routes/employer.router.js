    ...
    Employer.findOne({ resetPasswordToken: req.params.token })
        .then((user) => {
            ...
            `,
                                        ...
                                        mailController.transport.sendMail(
                                            resetEmail,
                                        );
                                    ...
                                    .catch((err) => {
                                        res.status(400).json({
                                            err: "Fields not unset",
                                        });
                                    });
                            })
                            .catch((err) => {
                                res.status(400).json({
                                    err: "Password not saved",
                                });
                            });
                    })
                    .catch((err) => {
    ...
    User.findById(req.body.id)
        .then((user) => {
            Job.findById(req.params.id)
                .then((job) => {
                    if (
                        job.description.count - job.acceptedApplicants.length ==
                        0
                    )
    ...
    Job.findById(req.params.id)
        .then((job) => {
            if (job.extension == 0)
                return res
                    .status(400)
                    .json({ err: "Job cannot be extended again" });
            job.extension = 0;
            job.expireAt = expiry;
            //console.log(job);
            job.save()
        ...
        Employer.findById(req.user._id)
            .then((employer) => {
                ...
                if (employer.sponsors.allowed - employer.sponsors.posted <= 0)
                    return res
                        .status(400)
                        .json({ err: "No sponsors remaining" });
                Employer.findByIdAndUpdate(req.user._id, {
                    $inc: { "sponsors.posted": 1 },
                })
                    .then((employer) => {
            ...
            .then((employer) => {
                ...
                if (employer.validated == false && employer.jobtier.posted > 0)
                    return res.status(400).json({
                        err:
                            "You can only post one Job until you are validated",
                    });
    ...
    if (days < 0 || days > 30)
        ...
        Employer.findById(req.user._id)
            .then((employer) => {
                var update = {};
                if (
                    employer.validated == false &&
                    employer.freelancetier.posted > 0
                )
                    return res.status(400).json({
                        err:
                            "You can only post one Job until you are validated",
                ...
                if (req.body.category == "Day Job") {
                    if (
                        employer.freelancetier.allowed -
                            employer.freelancetier.posted <=
                        0
                    )
                        return res.status(400).json({ err: "Max Jobs Posted" });
                ...
                Employer.findByIdAndUpdate(req.user._id, { $inc: update })
                    .then((employer2) => {
    ...
    Employer.findById(req.user._id)
        .then((employer) => {
            ...
            if (employer.validated == false && employer.jobtier.posted > 1)
                return res.status(400).json({
                    err: "Can only post one job until you are validated",
                });
            savedJob
                ...
                .then((sjob) => {
                    ...
                    if (days < 0 || days > 90)
                        return res
                            .status(400)
                            .send("Invalid time range spcified");
                    if (employer.jobtier.allowed - employer.jobtier.saved <= 0)
                        return res.status(400).send("Max Jobs Saved");
                    else
                        Employer.findByIdAndUpdate(req.user._id, {
    ...
    (req, res) => {
        Employer.findById(req.user._id)
            .then((employer) => {
                ...
                if (
                    employer.validated == false &&
                    employer.freelancetier.posted > 1
                )
                    return res.status(400).json({
                        err: "Can only post one job until you are validated",
                    });
        ...
        Freelance.findById(req.params.id)
            .then((job) => {
                if (req.body.startDate)
                    query.update["startDate"] = req.body.startDate;
                if (req.body.endDate)
                    query.update["endDate"] = req.body.endDate;
                if (req.body.attachedApplicants)
                    query.update["attachedApplicants"] =
                        req.body.attachedApplicants;
                const expiry = new Date(req.body.endDate);
                var days = (expiry - job.createdAt) / (1000 * 60 * 60 * 24);
                if (days < 0 || days > 90)
